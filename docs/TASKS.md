...
- [x] **C4. Profil ekranı** — kredi geçmişi (ledger), itibar, verilen/alınan sayıları, niş değiştirme (ayda 1), çıkış, hesap silme (Apple zorunlu).
...
- [x] **E5. Masaüstünden test açma** — web'de thumbnail seçme (tarayıcıda yeniden boyutlandırma) ve klip yükleme: ≤60 sn ve ≤8 MB ise kabul, değilse net hata + dışa aktarma önerisi. Sunucu tarafı transcoding YOK. Gerekçe: üreticiler kurguyu PC'de yapıyor; dosyayı telefona taşıtmak gerçek bir kayıp noktası. Kabul: masaüstü tarayıcıdan test açılabiliyor, 60 sn üstü ve 8 MB üstü dosya reddediliyor, mobil akış bozulmuyor.
...
- [x] **E6. Arayüz tasarımı** — `docs/DESIGN.md` sözleşmesi + tasarım tokenları (`src/design/tokens.ts`) ve ortak bileşenler (`Screen`, `Card`/`Rule`, `Section`, `Type`/`Stat`). Şablon mavisi (`#2f95dc`) kaldırıldı; sekme çubuğu ve başlık çubuğu da paletten besleniyor. Değerlendirme akışının feed adımı bilerek değiştirilmedi (DESIGN.md §2). Kabul: tüm ekranlar tokenlardan besleniyor, `Themed` yalnızca FeedStep'te kaldı, lint + 79 birim testi + `check:web` yeşil, sonuç ekranı tarayıcıda doğrulandı.
- [x] **E4. Ölçüm** (kod tarafı; PostHog hesabı + panel SENIN-YAPACAKLARIN §6) — PostHog olayları: `submission_created`, `task_started`, `review_submitted`, `review_rejected`, `paywall_viewed`, `purchase`. Dashboard: submission başına 24 saatte değerlendirme.
