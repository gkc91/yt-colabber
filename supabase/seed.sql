...
-- 3 test kullanıcısı, animation nişi. Giriş: <ad>@firstcut.test / password123
...
select '00000000-0000-0000-0000-000000000000', u.id, 'authenticated', 'authenticated', u.email,
       ...
       '', '', '', ''
from (values
  ('11111111-1111-1111-1111-111111111111'::uuid, 'alice@firstcut.test', 'Alice'),
  ('22222222-2222-2222-2222-222222222222'::uuid, 'bob@firstcut.test',   'Bob'),
  ('33333333-3333-3333-3333-333333333333'::uuid, 'cara@firstcut.test',  'Cara')
) as u(id, email, name);
...
select gen_random_uuid(), u.id, u.id::text,
       ...
       'email', now(), now(), now()
from auth.users u where u.email like '%@firstcut.test';
